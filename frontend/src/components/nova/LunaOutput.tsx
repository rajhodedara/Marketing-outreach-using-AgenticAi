...
export function LunaOutput({ data }: { data: LunaPlanData }) {
  ...
  return (
    ...
    >
      ...
      {data.customResponse ? (
        <motion.div variants={itemVariants} className="liquid-glass rounded-[2rem] p-8 diffusion-shadow relative overflow-hidden">
          <div className="absolute -top-24 -right-24 w-48 h-48 bg-emerald-500/10 rounded-full blur-[60px] pointer-events-none" />
          <h4 className="text-[11px] uppercase tracking-[0.15em] font-semibold text-emerald-400 mb-5 flex items-center gap-2 relative z-10">
            <Target weight="duotone" size={16} />
            Command Center Analysis
          </h4>
          <div className="prose prose-invert prose-sm max-w-none text-zinc-300 leading-relaxed relative z-10 font-sans">
            {data.customResponse.split('\n').map((line, i) => (
